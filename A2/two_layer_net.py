...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    scores = torch.matmul(X ,W1) + b1
    scores[scores < 0] = 0
    hidden = scores
    scores = torch.matmul(scores , W2) + b2
    # socres += (reg * torch.sum(W1 * W1) torch.sum(W2 * W2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    div = torch.sum(scores , axis=1).reshape(-1,1)
    scores /= div
    grad = scores
    index = torch.arange(0,X.shape[0],1)
    loss = -(torch.sum(torch.log(scores[index ,y])))
    loss /= X.shape[0]
    ...
    grad[index, y] -= 1
    grad /= num_train
    grads['W2'] = torch.matmul(h1.T,grad) + 2 * reg * W2
    grads['b2'] = torch.sum(grad,axis=0)
    dX =  torch.matmul(grad,W2.T)
    dX[h1 == 0] = 0
    grads['W1'] = torch.matmul(X.T ,dX) + 2 * reg * W1
    grads['b1'] = torch.sum(dX,axis=0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        # Replace "pass" statement with your code
        for param in params:
            moment1[param] = beta1 * moment1[param] + (1 - beta1) * grads[param]
            moment2[param] = beta2 * moment2[param] + (1 - beta2) * grads[param] * grads[param]
            moment1_unbias = moment1[param] / (1 -beta1 ** (it + 1))
            moment2_unbias = moment2[param] / (1 - beta2 ** (it + 1))
            params[param] -=  learning_rate * moment1_unbias / (moment2_unbias.sqrt() + 1e-7)
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    y_pred  = torch.matmul(X , params['W1']) + params['b1']
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
    y_pred[y_pred < 0] = 0
    y_pred = torch.matmul(y_pred , params['W2']) + params['b2']
...
def nn_get_search_params():
    ...
    regularization_strengths = [1e-2,1e-4,3e-1,2e-1,1e-1]
...
def find_best_net(
    ...
    for lr in learning_rates:
        for hs in hidden_sizes:
            for rs in regularization_strengths:
                ...
                stat = model.train(data_dict['X_train'],data_dict['y_train'],data_dict['X_val'],data_dict['y_val'],learning_rate=lr,reg=rs,num_iters=2000)
                val_acc = model.predict(data_dict['X_val'])
                train_acc = model.predict(data_dict['X_train'])
                val_acc = torch.mean((val_acc == data_dict['y_val']).float())
                train_acc =torch.mean((train_acc == data_dict['y_train']).float()) 
                if val_acc >  best_val_acc :
                    best_val_acc = val_acc
                    best_stat = stat
                    best_net = model
                print(f"lr: {lr} hs: {hs} rs: {rs} val_acc: {val_acc} train_acc: {train_acc}")
                results.append({
                'lr': lr,
                'reg': rs,
                'hs': hs,
    ...
    return best_net, best_stat, best_val_acc,results
