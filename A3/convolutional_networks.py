...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        value = torch.nn.functional.pad(x , (pad , pad , pad , pad),mode='constant',value=0)
        N,C,H,W = x.shape
        F,C,HH,WW = w.shape
        ...
        for n in range(N):
            for i in range(0,H_):
                for j in range(0,W_):
                  for f in range(F):
                      filters = w[f]
                      pic = value[n,:,i*stride:i*stride+HH,j*stride:j*stride+WW]
                      val = filters * pic
                      val = torch.sum(val)
                      out[n,f,i,j] = val + b[f]
    ...
    def backward(dout, cache):
        ...
        N,C,H,W = x.shape
        F,C,HH,WW = w.shape
        _,_,rows,cols = dout.shape
        dw = torch.zeros_like(w)
        db = torch.sum(dout , axis=0)
        db = torch.sum(db,axis=1)
        db = torch.sum(db,axis=1)
        ...
        for n in range(N) :
          for f in range(F):
            for c in range(C):
                value = torch.zeros(HH,WW,dtype=dout.dtype,device=dout.device)
                temp = x[n,c]
                temp_w = w[f,c]
                for row in range(rows):
                    for col in range(cols):
                        val = dout[n,f,row,col] * temp[row*stride:row*stride + HH , col*stride:col*stride+WW] 
                        dx[n,c,row*stride:row*stride + HH , col*stride:col*stride+WW]  +=  dout[n,f,row,col] * temp_w 
                        value += val
                dw[f,c] += value
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
            for i in range(H_):
                for j in range(W_):
                    for c in range(C):
                        max_val = x[n,c,i*stride : i * stride + pool_height , j * stride : j * stride + pool_width]
                        out[n,c,i,j] = torch.max(max_val)           
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
        ...
        for n in range(N):
            for i in range(H):
                for j in range(W):
                    for c in range(C):
                        max_val = x[n,c,i*stride : i * stride + pool_height, j * stride : j * stride + pool_width]
                        index  = torch.argmax(max_val)
                        coords = torch.unravel_index (index ,max_val.shape)
                        dx[n,c,coords[0].item() + i * stride,coords[1].item() + j * stride] += dout[n,c,i,j]
        #####################################################################
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,4):
            s = 'W'+str(i)
            loss += self.reg * torch.sum(self.params[s] ** 2)
        dout,grads['W3'] ,grads['b3'] = Linear().backward(dx ,cache3)
    ...
    def __init__(self,
        ...
        for layer in range(self.num_layers):
            ...
            if layer == self.num_layers - 1:
                flatten = channel * row * col
                if weight_scale == 'kaiming':
                  self.params[w] = kaiming_initializer(flatten , num_classes ,K=None,relu=False,device=device,dtype=dtype)
                else:
                  self.params[w] = torch.randn(flatten , num_classes , dtype=dtype , device=device) * weight_scale
                self.params[b] = torch.zeros(num_classes, dtype=dtype , device=device) 
            else:
                if weight_scale == 'kaiming':
                  self.params[w] = kaiming_initializer(num_filters[layer], channel ,K=3,relu=True,device=device,dtype=dtype)
                else:
                  self.params[w] = torch.randn(num_filters[layer],channel,3,3,dtype=dtype,device=device) * weight_scale
                self.params[b] = torch.zeros(num_filters[layer],dtype=dtype,device=device) 
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers):
            ...
            if layer == self.num_layers - 1:
               ...
               scores , cache = Linear().forward(scores,self.params[w],self.params[b])
               caches.append(cache)
            else:
              if layer in self.max_pools:
                scores ,cache = Conv_ReLU_Pool().forward(scores ,self.params[w],self.params[b] , conv_param , pool_param)
                caches.append(cache)
              else:
                scores ,cache = Conv_ReLU().forward(scores ,self.params[w],self.params[b] , conv_param)
                caches.append(cache)
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        cache = caches.pop()
        dx , grads[w],grads[b] = Linear().backward(dx ,cache)
        ...
        for layer in range(self.num_layers - 2 , -1 , -1):
            ...
            b = 'b' + str(layer + 1)
            if layer in self.max_pools:
               dx,grads[w] ,grads[b] = Conv_ReLU_Pool().backward(dx,cache) 
               grads[w] += 2 * self.reg * self.params[w]
            else :
               dx,grads[w] ,grads[b] = Conv_ReLU().backward(dx,cache) 
               grads[w] += 2 * self.reg * self.params[w]
        #############################################################
      #                       END OF YOUR CODE                    #
       #############################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    model = DeepConvNet(num_filters=[4, 8, 12, 16,24,32,48],max_pools = [2,3,5],weight_scale='kaiming', dtype=dtype, device=device,reg=7e-4)
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        weight = torch.randn(Din , Dout ,K,K,dtype=dtype , device=device) * weight_scale
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
