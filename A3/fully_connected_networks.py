...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss = scores[indices , y]
        loss = -torch.sum(torch.log(loss)) / N 
        loss +=  self.reg * (torch.sum(self.params['W1'] ** 2)  + torch.sum(self.params['W2'] ** 2))
        grad[indices , y] -= 1
        dx , dw ,db = Linear.backward(grad , cache2)
        dx /= N
        dw /= N
        grads['W2'] = dw 
        grads['W2'] += 2 * self.reg * self.params['W2']
        grads['b2'] = db / N
        dx , dw , db =Linear_ReLU.backward(dx , cache1)
        grads['W1'] = dw
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            b = 'b' + str(i+1)
            if i == 0:
               self.params[w] = torch.randn(input_dim,hidden_dims[i],dtype=dtype,device=device) * weight_scale
               self.params[b] = torch.zeros(hidden_dims[i],dtype=dtype,device=device)
            elif i == self.num_layers - 1:
               self.params[w] = torch.randn(hidden_dims[i-1],num_classes,dtype=dtype,device=device) * weight_scale
               self.params[b] = torch.zeros(num_classes,dtype=dtype,device=device)
            else:
               self.params[w] = torch.randn(hidden_dims[i-1],hidden_dims[i],dtype=dtype,device=device) * weight_scale
               self.params[b] = torch.zeros(hidden_dims[i],dtype=dtype,device=device) * weight_scale
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(n , -1 , -1):
            cache = caches.pop()
            if i == n :
              grad, dw ,db = Linear.backward(grad , cache)
              grad /= N
              dw /= N
              db /= N
            else:
               grad,dw,db = Linear_ReLU.backward(grad,cache)
            w = 'W'+str(i + 1)
            b = 'b'+str(i + 1)
            grads[w] = dw 
...
def create_solver_instance(data_dict, dtype, device):
    model = TwoLayerNet(hidden_dim=400, dtype=dtype, device=device)
    #############################################################
...
def sgd_momentum(w, dw, config=None):
    ...
    if 'velocity' not in config:
       config['velocity'] = config.get('velocity', torch.zeros_like(w))
